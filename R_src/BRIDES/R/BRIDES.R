							...
							if (found) {
								...
								if (length(list_of_knames)>0)
									...
									if (deadend) {
										...
										paths2<-get.all.shortest.paths(g2_without_k_and_i,g1names[j],kname, mode="out")$res;
												...
												if (intersect) {												
													...
													for (q in paths2) {
														...
														if(any(V(g2_without_k_and_j)[p] %in% V(g2_without_k_and_i)[q])) {
															...
															paths3<-get.all.shortest.paths(g2_without_k_and_p2,g1names[j],kname, mode="out")$res;
															if (length(paths3)>0) {
...
BRIDES<-function(g1,g2=NULL,node1="default", node2="default",taxnames='default',npath=0, maxdistance=0, maxtime=3600,maxnode=0,maxcores=1, outfile="",randomize=F, size=1000, first=0, last=0, sample_paths=c()) 
{
    #####################################################
	## Wrapper
