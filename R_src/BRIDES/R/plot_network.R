...
save_network<-function(g1,g2=NULL, filename, layout=layout.kamada.kawai, taxnames='', mode='png', imagesize=800) {	
	#g1 = blue
	#g2 = orange
	if (is.null(g2)) {
		g2=g1
	...
	if (taxnames=='') {
		...
		V(g1)$color <-"white";
	} else if (taxnames=='all'&&!is.null(V(g2)$name)&&!is.null(V(g2)$tax)) {					
			i=1;
			nv=c();
			cv=c();
	...
	if (length(V(g2)) > 50) {
		...
		if (mode=='png') {
			...
			legend("bottom", inset = c(0,-0.1),bty="n",horiz=TRUE,legend=nv,pt.bg=cv, pt.cex=2, col="black",pch=21, yjust=0, lty=0);
		     par(o)
			 a=dev.off();		
		} else if (mode=='svg') {
			...
			legend("bottom", inset = c(0,-0.1),bty="n",horiz=TRUE, legend=nv,pt.bg=cv, pt.cex=2, col="black",pch=21, yjust=0, lty=0,xpd=NA);
			par(o)
			a=dev.off();		
		} else if (mode=='eps'){
			...
			postscript(f2, horizontal = FALSE, fonts=c("serif", "Palatino"), onefile = FALSE, paper = "special", height = 8, width = 10)
				...
				legend("bottom", inset = c(0,-0.1),bty="n",horiz=TRUE,legend=nv,pt.bg=cv, col="black",pch=21, yjust=0, lty=0);
				par(o)
		}		
...
save_network_big<-function(g1,g2, filename, layout=layout.kamada.kawai, taxnames='', mode='png', imagesize=2500) {		
	...
	if (mode=='png') {
			...
			legend("bottom", inset = c(0,-0.1),bty="n",horiz=TRUE,title="Legend", legend=nv,pt.bg=cv, pt.cex=2, col="black",pch=21, yjust=0, lty=0,xpd=NA);
			par(o)
			a=dev.off();
	} else if (mode=='eps') {
			f2<-paste(filename,"_2",".eps", sep="");
...
plot_network<-function(g1,g2=NULL,layout=layout.kamada.kawai, taxnames='') {		
	save_network(g1,g2,filename='',taxnames=taxnames, layout=layout,mode='screen');
}