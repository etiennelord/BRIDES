...
library(fastmap)
  ...
  optimize_ga=function(size=2,iters=50,evalFunc=NULL, mutationChance=0.10, maxgenes=0, mingenes=1) {
    ...
    create_sample_path <- function(total_n, offset, size,directed=FALSE){
            path <- fastmap::faststack(init=(total_n*(total_n-1)/2))
            if (!directed) {
              t=0
              for(j in 1:total_n)  {
                for (i in j:total_n){
    ...
    # Total of new nodes in network Y: 5 
    # Number of edges in network Y: 15 
    # Number of nodes in network Y: 15 
    # Number of nodes in network X: 10 
    # Total of pathways to investigate: 45 
    # Clustering coefficient network Y: 0.24 
    # Clustering coefficient network X: 0 
    # Average degree ± std in network Y: 2 ± 1.195229 
    # Average degree ± std in network X: 1 ± 1.054093 
    # Average path length in network Y: 2.375 
    # Average path length in network X: 2.133333 
    # Number of clusters in network Y: 4 
    # Number of clusters in network X: 5 
    # Average cluster size ± std in network Y: 3.75 ± 4.856267 
    # Average cluster size ± std in network X: 2 ± 2.236068 
