...
#library(SDDE)
 ...
 ###################################################
  ...
  optimize_ga=function(size=2,iters=50,evalFunc=NULL, mutationChance=0.10, maxgenes=0, mingenes=1) {
	...
	###################################################
	#function create_sample_path
	# This function create some combination of path (src, dst)
	# as a vector. Numbering is from 1.
	# total_n: number of nodes in network
	# offset : starting position of the pathways if we don't start at the first one
    #          note: the total number of offset is ((total_n*(total_n-1)/2)/size)+1
	#                and we start the numbering at 1 instead of 0
	# size   : size of sampling (returned array)
	create_sample_path <- function(total_n, offset, size,directed=FALSE){
			if (!directed) {
				return(.C("createSamplePath", as.integer(total_n), as.integer(offset), as.integer(size), path = integer(size*2))$path);
			}
			if (directed) {
				 path=c();
				 t=0
	...
	export_network<-function(g,file,attributes=FALSE){
		if (!is.igraph(g)) {
			export_network(g$g1,paste(file,"_g1.txt",sep=""), attributes);
			export_network(g$g2,paste(file,"_g2.txt",sep=""), attributes);
		} else {
	...
	load_network<-function(filename_or_df, filename_attributes_or_df='', edge_weight='equal',sep="\t",header=FALSE, directed=FALSE) {	
		if (!(edge_weight %in% c('equal','proportional','inverse'))) {
			warning("Edge_weight parameters must be either 'equal' (unweigthed), 'proportional' (weigthed) or 'inverse' (1/weight) \n")
			return (NULL)
		}
		...
		if (!is.data.frame(filename_or_df)) {
			...
			if (tsep=='\t') tsep="tab"
			if (tsep==' ') tsep="space"
			cat("Loading network from ",filename_or_df," using ", tsep, " as separator\n", sep="");
			if (!is.data.frame(filename_attributes_or_df)&&filename_attributes_or_df!='') {
				cat("Loading network attributes from ", filename_attributes_or_df, "\n", sep="");
			}
			cat("Directed    :",directed,"\n");
			dS=read.delim(filename_or_df,sep=sep, header=header);
		} else {
			cat("==========================================================\n");
			cat("Loading network from data.frame\n");
			cat("Directed    :",directed,"\n");
			dS=filename_or_df;
		}
		#Make sure that we keep only two columns when creating the graph
		gS=graph.data.frame(dS[,1:2],directed=directed);
		if (filename_attributes_or_df!='') {
			if (!is.data.frame(filename_attributes_or_df)) {
				dS2=read.delim(filename_attributes_or_df,sep='\t',header=header, row.names=1);
			} else {
				cat("Using attributes from data.frame\n", sep="");
				dS2=filename_attributes_or_df;
			}
			V(gS)$tax=as.character(dS2[V(gS)$name,1]) 
		}
		
		#equal, proportional, or inverse
		if (ncol(dS)>2&&edge_weight=='proportional') {
			E(gS)$weight=dS[,3];
			cat("Weighted    : proportional\n");
		} else
		if (edge_weight=='equal') {
		cat("Weighted    : equal (=1.0)\n");
			E(gS)$weight=1.0;
		} else
		if (edge_weight=='inverse') {
			E(gS)$weight=dS[,3];	
			E(gS)$weight=1/E(gS)$weight;
			cat("Weighted    : inverse\n");
		}
...
pathBRIDES<-function(g1,g2_without_k,g1names,g2_unique_names_primed,g2_unique_number_primed,node1_number,node2_number,no_new_node,maxdistance,maxtime,maxnode,t0, disable_trace=FALSE) {
	#options(warn=-1); #disable warnings since some vertex could become unreachable
	###################################################
    ## Flags
	verbose<-FALSE;
	trace<-TRUE;
	if (disable_trace) trace<-FALSE #Speed up
...
################################################
	...
	if (outfile!="") write("src\tdest\tdist_x\tdist_y\tBRIDES\tpath\tpath_attributes",outfile)
		...
		for (p in first:last) {
			...
			s<-foreach(hi=1:npath, .combine=function(i,j, .export=c("pathways")) { return(i+j)}) %dopar% {
				if(!exists("pathBRIDES", mode="function")) library(BRIDES)
				i=sample_paths[(hi-1)*2+1];
				j=sample_paths[(hi-1)*2+2];				
				b=pathBRIDES(g1,g2_without_k,g1names,g2_unique_names_primed,g2_unique_number_primed,i,j,no_new_node,maxdistance,maxtime,maxnode,proc.time(), disable_trace=!path_information);
				a=array(0,6)
				a[b$path_type0]=1;
	...
	# CASE 2. We have one sample 
		...
		options(warn=1)
		pathBRIDES(g1,g2_without_k,g1names,g2_unique_names_primed,g2_unique_number_primed,node1_number,node2_number,no_new_node,maxdistance,maxtime,maxnode,proc.time(), disable_trace=!path_information);
	}
	
}

	###################################################
	#This function return information about good path
	#between two nodes in this network, i.e. is there a 
	#path?
